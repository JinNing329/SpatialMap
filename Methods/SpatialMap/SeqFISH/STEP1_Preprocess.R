...
### ST:SeqFISH
SeqFISH <- as.matrix(fread("cortex_svz_counts.csv"))
SeqFISHMeta<- as.data.frame(fread("cortex_svz_cellcentroids.csv"))
############### match spatial with sc
scGeneName = rownames(counts)
STGeneName = colnames(SeqFISH)
ShareGene =intersect(scGeneName, STGeneName)
SeqFISH=SeqFISH[,match(ShareGene,STGeneName)]
counts=counts[match(ShareGene, scGeneName),]
rowMeta=rowMeta[match(ShareGene, scGeneName),]
saveRDS(SeqFISH, file = "./objects/ST.rds")
...
CorrelationMatrix = cor(x=t(countsMean),y=t(SeqFISH))
...
# kernel function
library("KRLS")
coordinates<-matrix(c(SeqFISHMeta$X, SeqFISHMeta$Y), ncol = 2, byrow = FALSE)
