...
sig=c(paste0("sig",1:5),paste0("car",1:9))[index1]
  ...
  cat(paste0("--------------",sig,"_",index2,"-------------",'\n'))
  ######################### read data #################
  y <- readRDS("./ST.rds")[,((index2-1)*100+1):(index2*100)]
  y <-t(y)
  genotype <- readRDS("./X.rds")[,((index2-1)*100+1):(index2*100)]
  genotype <-t(genotype)
  genename=rownames(y)
  m=nrow(y)
  n=ncol(y)
  ...
  for (i in 1:m) {
    ...
    res <- pqlseq(RawCountDataSet=count, Phenotypes=predictor,
                      ...
                      fit.tol=1e-4,fit.model="PMM",numCore=4)
    result=rbind(result, res)
  }
  path <- paste0(sig,"__",index2,".rds")
  saveRDS(result,path) 
}
