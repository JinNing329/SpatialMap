...
pqlseq <- function(RawCountDataSet, Phenotypes, Covariates=NULL, RelatednessMatrix=NULL, LibSize=NULL, 
	...
	if(fit.model == "PMM"){
		...
		resPMM <-foreach(iVar=1:numVar,.combine=rbind)%dopar%{
			numAnalysis <- beta <- alpha <-tau1 <- tau2 <- se_beta <- pvalue <- converged <- h2 <- sigma2 <- overdisp <- NA
			if(numCov==0){
			...
			res <- data.frame(numIDV = numAnalysis, alpha=alpha, beta = beta, se_beta = se_beta, 
